...
import { CheckSquare, LayoutDashboard, LogOut, Users } from '@lucide/vue'
...
<template>
  <div class="flex min-h-screen bg-gray-50">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      <AppHeader :title="title" :subtitle="subtitle" @toggle="sidebarOpen = !sidebarOpen">
        <template #actions>
          <slot name="actions" />
          <div class="ml-1 flex items-center gap-x-2 border-l border-gray-200 pl-3">
            <span class="hidden max-w-[180px] truncate text-xs text-gray-500 sm:block">
              {{ auth.user?.email }}
            </span>
            <button
              type="button"
